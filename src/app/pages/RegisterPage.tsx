...
export function RegisterPage() {
  ...
  const steps = [
    ...
    {
      id: 1,
      title: language === 'fr' ? 'Compte' : language === 'de' ? 'Konto' : 'Account',
      icon: Lock,
      description: language === 'fr' ? 'Création du compte' : language === 'de' ? 'Kontoerstellung' : 'Account creation'
    },
    {
      id: 2,
      title: language === 'fr' ? 'Vérification' : language === 'de' ? 'Verifizierung' : 'Verification',
      icon: ShieldCheck,
      description: language === 'fr' ? 'Sécurisation du compte' : language === 'de' ? 'Kontosicherung' : 'Account security'
    },
    {
      id: 3,
      title: language === 'fr' ? 'Confirmation' : language === 'de' ? 'Bestätigung' : 'Confirmation',
      icon: CheckCircle2,
      description: language === 'fr' ? 'Finalisation' : language === 'de' ? 'Abschluss' : 'Finalization'
    }
  ];
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 1) {
      // Validation Account (All inputs: Email, Phone, Password, Terms)
    ...
    if (step === 2) {
      // Verification Step
      if (!isEmailVerified) {
        newErrors.emailVerification = language === 'fr' ? 'Veuillez vérifier votre email' : language === 'de' ? 'Bitte verifizieren Sie Ihre E-Mail' : 'Please verify your email';
      }
      if (!isPhoneVerified) {
        newErrors.phoneVerification = language === 'fr' ? 'Veuillez vérifier votre téléphone' : language === 'de' ? 'Bitte verifizieren Sie Ihre Telefonnummer' : 'Please verify your phone';
      }
    }
    ...
    if (validateStep(currentStep)) {
      ...
      if (currentStep === 2) {
        if (isEmailVerified && isPhoneVerified) {
          ...
          // Proceed to final step (Confirmation)
        } else {
          // Errors are handled in validateStep mostly, but extra toast here is fine
          toast.error(
            language === 'fr'
  ...
  const sendPhoneVerificationCode = () => {
    toast.success(
      language === 'fr'
        ? `Code de vérification envoyé au ${formData.phone}`
        : language === 'de'
          ? `Verifizierungscode an ${formData.phone} gesendet`
  ...
  const handleVerifyEmail = async () => {
    try {
      const userverifydata = new FormData();
      userverifydata.append('email', formData.email);
      userverifydata.append('code', emailVerificationCode);
      const response: any = await apiClient.post<any>('/auth/verify-email', userverifydata);
  ...
  const handleVerifyPhone = async () => {
    try {
      ...
      const userverifydata = new FormData();
      userverifydata.append('email', formData.phone);
      userverifydata.append('code', phoneVerificationCode);
      const response: any = await apiClient.post<any>('/auth/verify-phone', userverifydata);
      ...
      if (response.data) {
        ...
        toast.success(
          language === 'fr'
            ...
            : language === 'de'
              ? 'Telefon erfolgreich verifiziert'
              ...
              return (
                <div key={step.id} className="flex items-center flex-1">
                  ...
                  {index < steps.length - 1 && (
                    <div
                      className={`h-1 flex-1 mx-2 mt-[-40px] transition-all ${isCompleted ? 'bg-emerald-500' : 'bg-gray-200'
                        }`}
                    />
                  )}
                </div>
          ...
          <Card className="max-w-3xl mx-auto shadow-2xl border-2">
            <CardHeader className="bg-gradient-to-r from-blue-600 via-purple-600 to-emerald-500 text-white">
              <div className="flex items-center justify-between">
                ...
                {currentStep === 3 && getAccountStatusBadge()}
              </div>
            </CardHeader>
            ...
            <CardContent className="p-8">
              <AnimatePresence mode="wait">
                ...
                {/* Step 1: Account Creation (Contact + Security) */}
                {currentStep === 1 && (
                  <motion.div
                    key="step-1"
                    initial={{ opacity: 0, x: 20 }}
                  ...
                  >
                    <div className="mb-6">
                      <h3 className="text-xl font-semibold text-gray-900 mb-2">
                        {language === 'fr' ? 'Création de compte' : language === 'de' ? 'Kontoerstellung' : 'Account Creation'}
                      </h3>
                      <p className="text-sm text-gray-600">
                        {language === 'fr'
                          ? 'Remplissez les informations ci-dessous pour créer votre compte'
                          : language === 'de'
                            ? 'Füllen Sie die untenstehenden Informationen aus, um Ihr Konto zu erstellen'
                ...
                {/* Step 2: Verification (Email + Phone) */}
                {currentStep === 2 && (
                  <motion.div
                    key="step-2"
                    initial={{ opacity: 0, x: 20 }}
                  ...
                  >
                    <div className="mb-6">
                      <h3 className="text-xl font-semibold text-gray-900 mb-2">
                        {language === 'fr' ? 'Vérification de compte' : language === 'de' ? 'Kontoverifizierung' : 'Account Verification'}
                      </h3>
                      <p className="text-sm text-gray-600">
                        {language === 'fr'
                          ? 'Veuillez saisir les codes de vérification envoyés à votre email et téléphone.'
                          : language === 'de'
                            ? 'Bitte geben Sie die Bestätigungscodes ein, die an Ihre E-Mail und Ihr Telefon gesendet wurden.'
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 space-y-4">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium text-gray-900">
                              {language === 'fr' ? 'Code Email' : language === 'de' ? 'E-Mail-Code' : 'Email Code'}
                            </p>
                      ...
                      {!isEmailVerified && (
                        <>
                          <div className="flex gap-2">
                            <Input
                              placeholder={language === 'fr' ? 'Code à 6 chiffres' : 'Code'}
                              value={emailVerificationCode}
                              onChange={(e) => setEmailVerificationCode(e.target.value)}
                              maxLength={6}
                            />
                            <Button onClick={handleVerifyEmail} disabled={emailVerificationCode.length < 4} variant="secondary" className="border">
                      ...
                      {errors.emailVerification && !isEmailVerified && (
                        <p className="text-sm text-red-600">{errors.emailVerification}</p>
                      )}
                    </div>
                    ...
                    <div className="bg-purple-50 border border-purple-200 rounded-lg p-6 space-y-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium text-gray-900">
                              {language === 'fr' ? 'Code Mobile' : language === 'de' ? 'Handy-Code' : 'Mobile Code'}
                            </p>
                        ...
                        <>
                          <div className="flex gap-2">
                            <Input
                              ...
                              maxLength={6}
                            />
                            <Button onClick={handleVerifyPhone} disabled={phoneVerificationCode.length < 4} variant="secondary" className="border">
                              {language === 'fr' ? 'Vérifier' : language === 'de' ? 'Verifizieren' : 'Verify'}
                            </Button>
                          </div>
                        </>
                      )}
                      {errors.phoneVerification && !isPhoneVerified && (
                  ...
                  >
                    {currentStep === 1 ? (
                      <>
                        ...
                        {language === 'fr' ? 'Créer le compte' : language === 'de' ? 'Konto erstellen' : 'Create Account'}
                      </>
                    ) : currentStep === 2 ? (
                      <>
                        <ShieldCheck className="w-4 h-4 text-white" />
                        {language === 'fr' ? 'Valider et Terminer' : language === 'de' ? 'Validieren und Beenden' : 'Validate and Finish'}
                      </>
                    ) : (
                      <>
